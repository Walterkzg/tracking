...
def main(input_path, output_path, progress_callback=None, job_id=None):
    # 加载配置
   ...
   # 初始化帧计数器
    ...
    while True:
        ...
        # 确保boxes和track_ids长度一致
            if len(boxes) != len(track_ids):
                print("[WARNING] 检测框与ID数量不匹配，跳过此帧")
                continue
    ...
    # 显示帧率信息
        ...
        if progress_callback and frame_count % 10 == 0:
            try:
                progress_callback(frame_count)  # 安全调用
            except Exception as e:
                print(f"[WARNING] 回调执行失败: {str(e)}")
...
if __name__ == "__main__":
    ...
    main(
        ...
        progress_callback=None,  # 命令行模式不需要回调
        job_id=None
    )