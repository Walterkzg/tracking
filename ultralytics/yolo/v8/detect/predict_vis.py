...
# ... [保留原有的工具函数，包括xyxy_to_xywh、compute_color_for_labels等] ...
...
class DetectionPredictor(BasePredictor):
    ...
    def setup_video_source(self):
        ...
        if self.args.source.endswith(('.mp4', '.avi', '.mov')):
            self.vid_cap = cv2.VideoCapture(self.args.source)
            if not self.vid_cap.isOpened():
                raise IOError(f"无法打开视频文件 {self.args.source}")
    ...
    def write_results(self, idx, preds, batch):
        ...
        if self.args.save and self.writer is None:
            # 优先从视频源获取fps
            fps = getattr(self, 'fps', None) or (
                int(self.vid_cap.get(cv2.CAP_PROP_FPS)) 
                if self.vid_cap else 30
            )
            
            h, w = im0.shape[:2]
            save_path = str(self.save_dir / 'output.mp4')
